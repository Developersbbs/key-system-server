...
exports.getMemberCourses = async (req, res) => {
  try {
    ...
    const completedCourseIdsForSync = user.completedCourses.map((id) =>
      id.toString(),
    );
    ...
    if (accessibleUpdated) {
      user.accessibleLevels = Array.from(new Set(user.accessibleLevels)).sort(
        (a, b) => a - b,
      );
      await user.save();
    }
    ...
    const allAccessibleCourses = unlockedLevels.flatMap(
      (level) => level.courses,
    );

    // ✅ NEW LOGIC: Only show courses that user can access sequentially
    const accessibleCourses = [];
    const userCompletedCourseIds = user.completedCourses.map((id) =>
      id.toString(),
    );
    ...
    for (const level of unlockedLevels) {
      ...
      if (completedCoursesInLevel < coursesInLevel.length) {
        ...
        if (
          nextCourse &&
          !userCompletedCourseIds.includes(nextCourse._id.toString())
        ) {
          accessibleCourses.push({
            ...nextCourse.toObject(),
            isCompleted: false,
...
exports.canAccessCourse = async (req, res) => {
  try {
    ...
    if (isCompleted) {
      return res
        .status(200)
        .json({ canAccess: true, reason: "Course already completed" });
    }
    ...
    if (!level) {
      return res
        .status(404)
        .json({ message: "Level not found for this course" });
    }
    ...
    const coursesInLevel = level.courses;
    const courseIndex = coursesInLevel.findIndex(
...
exports.getUserProgress = async (req, res) => {
  try {
    ...
    const progressSummary = unlockedLevels.map((level) => {
      ...
      const nextCourse =
        nextCourseIndex < totalCourses ? level.courses[nextCourseIndex] : null;

      // Calculate course progress based on chapter completion
      let totalChapters = 0;
      ...
      for (const course of level.courses) {
        ...
        if (completedCourseIds.includes(course._id.toString())) {
          completedChapters += course.chapters?.length || 0;
        }
      }
...
exports.updateCourse = async (req, res) => {
  try {
    const { title, description, category, image, isPublished, isApproved } =
      req.body;
    const course = await Course.findById(req.params.id);
    ...
    if (course) {
      ...
      course.isApproved =
        isApproved !== undefined ? isApproved : course.isApproved;
