...
exports.submitMcqResult = async (req, res) => {
  try {
    ...
    chapter.mcqs.forEach((mcq) => {
      if (
        answers[mcq._id] !== undefined &&
        Number(answers[mcq._id]) === mcq.correctAnswerIndex
      ) {
        correctAnswers++;
      }
    });
    ...
    const newResult = {
      ...
      attemptedAt: new Date(), // for recent order
    };
    ...
    if (isCourseComplete) {
      ...
      if (currentLevel) {
        ...
        const allCoursesInLevelCompleted = currentLevel.courses.every(
          (courseId) => completedCourseIds.includes(courseId.toString()),
        );
