...
userSchema.methods.canAccessCourse = function (
  courseId,
  coursePosition,
  levelCourses,
) {
...
userSchema.methods.getCourseCompletionPercentage = function (courseChapterIds) {
  const completedChapterIds = this.mcqResults.map((result) =>
  ...
  return courseChapterIds.length > 0
    ? Math.round((completedCount / courseChapterIds.length) * 100)
    : 0;
};
  ...
  return {
    ...
    progressPercentage:
      levelCourses.length > 0
        ? Math.round((completedCount / levelCourses.length) * 100)
        : 0,
...
userSchema.methods.getPublicPaymentDetails = function () {
  ...
  if (
    this.paymentDetails?.qrCodeUrl &&
    this.paymentDetails.qrCodeUrl.trim() !== ""
  ) {
  ...
  if (
    ...
    this.paymentDetails.accountHolderName.trim() !== ""
  ) {
  ...
  if (
    ...
    this.paymentDetails.accountNumber.trim() !== ""
  ) {
  ...
  if (
    this.paymentDetails?.ifscCode &&
    this.paymentDetails.ifscCode.trim() !== ""
  ) {
  ...
  if (
    ...
    this.paymentDetails.bankName.trim() !== ""
  ) {
